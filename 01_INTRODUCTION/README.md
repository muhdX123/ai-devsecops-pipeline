...
Existing SAST and DAST tools can generate a large number of false-positive results. As a result, developers and security engineers often need to manually review and validate flagged issues, which takes additional time and effort that could otherwise be spent on development activities.

#### Deployment Bottlenecks

Another challenge is the time and computational resources required for comprehensive dynamic security testing. When these scans take too long to complete, they can slow down pipeline execution and make it more difficult for development teams to maintain rapid release cycles.

#### Limited Context-Aware Detection
...
This research focuses on the automated detection of key web application vulnerabilities, specifically **SQL Injection (SQLi)** and **Cross-Site Scripting (XSS)**, within source code and HTTP request parameters.

The AI detection model will utilize supervised machine learning techniques trained on standard benchmark vulnerability datasets.

The prototype will be integrated into a containerized CI/CD environment, such as **GitHub Actions** or **GitLab CI**, to simulate a real-world DevSecOps execution workflow.

## Significance of Study

This study contributes to the field of software security and DevSecOps automation by demonstrating how machine learning algorithms can upgrade vulnerability detection without impeding deployment velocity.

The outcome benefits software developers, DevSecOps engineers, and security analysts by providing an intelligent, context-aware scanning mechanism that minimizes manual triage effort and accelerates secure software delivery.

## Proposed Direction

The proposed research focuses on combining AI-assisted vulnerability detection with automated DevSecOps execution. The main detection targets are **SQL Injection (SQLi)** and **Cross-Site Scripting (XSS)**, with evaluation based on detection accuracy, false-positive rate, and detection time.
